        ...
        void bwFillAttributes_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (AttributeMetadata amd in emd.Attributes)
            {
                    ...
                    && string.IsNullOrEmpty(amd.AttributeOf))
                {
                    ...
                    item.SubItems.Add((amd.IsCustomizable.Value || amd.IsManaged.Value == false).ToString());
                    ...
                    if (!amd.IsValidForAdvancedFind.CanBeChanged || (!amd.IsCustomizable.Value || amd.IsManaged.Value))
                    {
