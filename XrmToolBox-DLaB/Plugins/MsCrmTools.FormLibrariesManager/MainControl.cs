...
namespace MsCrmTools.FormLibrariesManager
{
    public partial class MainControl : UserControl, IMsCrmToolsPluginUserControl
    {
        ...
        public void UpdateConnection(IOrganizationService newService, ConnectionDetail detail, string actionName = "", object parameter = null)
        {
            ...
            if (actionName == "LoadLibrariesAndForms")
            {
                LoadLibrariesAndForms();
            }
        }
        ...
        private void tsbLoadLibrariesAndScripts_Click(object sender, EventArgs e)
        {
            if (service == null)
            {
                if (OnRequestConnection != null)
                {
                    var args = new RequestConnectionEventArgs { ActionName = "LoadLibrariesAndForms", Control = this, Parameter = null };
                    OnRequestConnection(this, args);
                }
            }
            else
        ...
        {
            infoPanel = InformationPanel.GetInformationPanel(this, "Loading libraries...", 340, 150);
        ...
        void worker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            worker.ReportProgress(0, "Loading forms...");
            ...
            worker.ReportProgress(0, "Filling controls...");
        }
        ...
        private void tsbAddCheckedScripts_Click(object sender, EventArgs e)
        {
            ...
            var addWorker = new BackgroundWorker {WorkerReportsProgress = true};
            addWorker.DoWork += addWorker_DoWork;
            addWorker.ProgressChanged += addWorker_ProgressChanged;
            addWorker.RunWorkerCompleted += addWorker_RunWorkerCompleted;
            addWorker.RunWorkerAsync(new object[] { sod.Scripts, sod.AddFirst, forms });
        }

        void addWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            var worker = (BackgroundWorker)sender;
            
            var scripts = (List<Entity>)((object[])e.Argument)[0];
            var addFirst = (bool)((object[])e.Argument)[1];
            var forms = (List<Entity>)((object[])e.Argument)[2];
            ...
            {
                worker.ReportProgress(0,string.Format("Updating form '{0}' ({1})...",form.GetAttributeValue<string>("name"), form.GetAttributeValue<string>("objecttypecode")));

                foreach (var script in scripts)
                {
                    try
                    {
                        fManager.AddLibrary(form, script.GetAttributeValue<string>("name"), addFirst);
            ...
            {
                worker.ReportProgress(0, string.Format("Publishing entity '{0}'...", entitiesToPublish));
                fManager.PublishForm(entitiesToPublish);
            }
        }

        void addWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            InformationPanel.ChangeInformationPanelMessage(infoPanel, e.UserState.ToString());
        }

        void addWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            Controls.Remove(infoPanel);
            infoPanel.Dispose();
        ...
        private void tsbRemoveCheckedScripts_Click(object sender, EventArgs e)
        {
            ...
            var removeWorker = new BackgroundWorker { WorkerReportsProgress = true };
            removeWorker.DoWork += removeWorker_DoWork;
            removeWorker.ProgressChanged += removeWorker_ProgressChanged;
            removeWorker.RunWorkerCompleted += removeWorker_RunWorkerCompleted;
            removeWorker.RunWorkerAsync(new[] { scripts, forms });
        }
        ...
        void removeWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            var scripts = (List<Entity>)((object[])e.Argument)[0];
            var forms = (List<Entity>)((object[])e.Argument)[1];
