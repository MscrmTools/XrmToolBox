    ...
    public partial class MainControl : UserControl, IMsCrmToolsPluginUserControl
    {
        ...
        private void tsbCreateNewParameter_Click(object sender, EventArgs e)
        {
            ...
            if (pForm.ShowDialog(this) == DialogResult.OK)
            {
                ...
                infoPanel = InformationPanel.GetInformationPanel(this, "Creating attribute...", 340, 150);

                var bwCreateAttribute = new BackgroundWorker{WorkerReportsProgress = true};
                bwCreateAttribute.DoWork += bwCreateAttribute_DoWork;
                bwCreateAttribute.ProgressChanged += bwCreateAttribute_ProgressChanged;
                bwCreateAttribute.RunWorkerCompleted += bwCreateAttribute_RunWorkerCompleted;
                bwCreateAttribute.RunWorkerAsync(new object[]{pForm.Parameter, lvForms.CheckedItems});
            }
        }

        void bwCreateAttribute_DoWork(object sender, DoWorkEventArgs e)
        {
            var parameter = (FormParameter)((object[]) e.Argument)[0];
            var lvItems = (ListView.CheckedListViewItemCollection)((object[])e.Argument)[1];
            ...
            // Publishing form
            InformationPanel.ChangeInformationPanelMessage(infoPanel, "Publishing form(s) ...");
            CrmForm.PublishForms(service, formsUpdated.Select(f => f.EntityLogicalName));

            e.Result = lvItems;
        }

        void bwCreateAttribute_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            InformationPanel.ChangeInformationPanelMessage(infoPanel, e.UserState.ToString());
        }

        void bwCreateAttribute_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            Controls.Remove(infoPanel);
            infoPanel.Dispose();
            ...
            {
                var lvItems = (ListView.CheckedListViewItemCollection)e.Result;
                foreach (ListViewItem item in lvItems)
                {
        ...
        private void tsbDeleteParameters_Click(object sender, EventArgs e)
        {
            ...
            infoPanel = InformationPanel.GetInformationPanel(this, "Deleting attribute(s)...", 340, 150);

            var bwDeleteAttribute = new BackgroundWorker { WorkerReportsProgress = true };
            bwDeleteAttribute.DoWork += bwDeleteAttribute_DoWork;
            bwDeleteAttribute.ProgressChanged += bwDeleteAttribute_ProgressChanged;
            bwDeleteAttribute.RunWorkerCompleted += bwDeleteAttribute_RunWorkerCompleted;
            bwDeleteAttribute.RunWorkerAsync(new object[] { lvParameters.SelectedItems });
        }

        void bwDeleteAttribute_DoWork(object sender, DoWorkEventArgs e)
        {
            var lvItems = (ListView.SelectedListViewItemCollection)((object[])e.Argument)[0];
            var formsUpdated = new List<CrmForm>();
            ...
            e.Result = formsUpdated;
        }

        void bwDeleteAttribute_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            InformationPanel.ChangeInformationPanelMessage(infoPanel, e.UserState.ToString());
        }

        void bwDeleteAttribute_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            Controls.Remove(infoPanel);
            infoPanel.Dispose();
            ...
            {
                foreach (var form in (List<CrmForm>) e.Result)
                {
