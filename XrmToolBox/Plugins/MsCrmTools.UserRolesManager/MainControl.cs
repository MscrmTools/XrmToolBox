...
namespace MsCrmTools.UserRolesManager
{
    public partial class MainControl : UserControl, IMsCrmToolsPluginUserControl
    {
        ...
        private void action1ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            infoPanel = InformationPanel.GetInformationPanel(this, "Adding roles to principals...", 340, 150);

            var bw1 = new BackgroundWorker { WorkerReportsProgress = true };
            bw1.DoWork += bw1_DoWork;
            bw1.ProgressChanged += bw1_ProgressChanged;
            bw1.RunWorkerCompleted += bw1_RunWorkerCompleted;
            bw1.RunWorkerAsync(ra);
        }

        void bw1_DoWork(object sender, DoWorkEventArgs e)
        {
            var worker = (BackgroundWorker)sender;
            var action = (RoleAction)e.Argument;

            var rManager = new RoleManager(service);
            rManager.AddRolesToPrincipals(action.Roles, action.Principals, AllRoles, worker);
        }

        void bw1_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            InformationPanel.ChangeInformationPanelMessage(infoPanel, string.Format(e.UserState.ToString(), e.ProgressPercentage));
        }
        ...
        private void action2ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            infoPanel = InformationPanel.GetInformationPanel(this, "Removing roles to users...", 340, 150);

            var bw2 = new BackgroundWorker { WorkerReportsProgress = true };
            bw2.DoWork += bw2_DoWork;
            bw2.ProgressChanged += bw2_ProgressChanged;
            bw2.RunWorkerCompleted += bw2_RunWorkerCompleted;
            bw2.RunWorkerAsync(ra);
        }
        ...
        void bw2_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            var rManager = new RoleManager(service);
            rManager.RemoveRolesFromPrincipals(action.Roles, action.Principals, AllRoles, worker);
        }
        ...
        private void action3ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            infoPanel = InformationPanel.GetInformationPanel(this, "Removing roles from principals...", 340, 150);

            var bw3 = new BackgroundWorker { WorkerReportsProgress = true };
            bw3.DoWork += bw3_DoWork;
            bw3.ProgressChanged += bw3_ProgressChanged;
            bw3.RunWorkerCompleted += bw3_RunWorkerCompleted;
            bw3.RunWorkerAsync(ra);
        }
        ...
        void bw3_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            worker.ReportProgress(0, "Adding roles to principals ({0} %)...");

            rManager.AddRolesToPrincipals(action.Roles, action.Principals, AllRoles, worker);
        }

        void bw3_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            InformationPanel.ChangeInformationPanelMessage(infoPanel, string.Format(e.UserState.ToString(), e.ProgressPercentage));
        }
